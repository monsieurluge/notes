    ...
    function Add() {
        ...
        this.install = function () {
            ...
            document.getElementById("enter").addEventListener("click", () => {
                ...
                let content = this.display.innerHTML.replace(
                    /\s?(<br\s?\/?>)\s?/g,
                    "\r\n",
                ); // replace line breaks
            ...
            for (var i = 0; i < this.keys.length; i++) {
                this.elementList[this.keys[i]].elem = document.getElementById(
                    this.elementList[this.keys[i]].key,
                );
                this.elementList[this.keys[i]].elem.oninput =
                    this.onElemChanged;
                this.elementList[this.keys[i]].elem.onfocus = this.onElemFocus;
                this.elementList[this.keys[i]].elem.onblur = this.onElemBlur;
                this.elementList[this.keys[i]].elemKey =
                    document.getElementById(
                        "key" + this.elementList[this.keys[i]].key,
                    );
            }
            setTimeout(function () {
                parent.setupData();
            ...
            for (var i = 0; i < parent.keys.length; i++) {
                if (e.target.id == parent.elementList[parent.keys[i]].key) {
                    if (!parent.elementList[parent.keys[i]].added) {
                        ...
                        parent.elementList[
                            parent.keys[i]
                        ].elemKey.style.visibility = "visible";
                    }
                    break;
                }
            }
        ...
        this.onElemBlur = function (e) {
            for (var i = 0; i < parent.keys.length; i++) {
                if (e.target.id == parent.elementList[parent.keys[i]].key) {
                    if (
                        parent.elementList[parent.keys[i]].elem.value == "" &&
                        parent.elementList[parent.keys[i]].added
                    ) {
                        parent.elementList[parent.keys[i]].added = false;
                        parent.elementList[
                            parent.keys[i]
                        ].elemKey.style.visibility = "hidden";
                    }
                    break;
                }
            }
        ...
        this.setupData = function () {
            ...
            for (var i = 0; i < parent.keys.length; i++) {
                if (parent.elementList[parent.keys[i]].key == "Title") {
                    ...
                    value += "&nbsp;&nbsp;";
                    value +=
                        parent.elementList[parent.keys[i]].desc.toUpperCase() +
                        " : ";
                    ...
                    if (parent.elementList[parent.keys[i]].type == "lower") {
                        ...
                        parent.elementList[parent.keys[i]].type == "text"
                    ) {
                        value += parent.elementList[parent.keys[i]].elem.value;
                    } else if (
                        parent.elementList[parent.keys[i]].type == "url"
                    ) {
                        // TODO: validate
                        value += parent.elementList[parent.keys[i]].elem.value;
                    } else if (
                        parent.elementList[parent.keys[i]].type == "tags"
                    ) {
                        // TODO: Format
                        value += parent.elementList[parent.keys[i]].elem.value;
                    } else if (
                        parent.elementList[parent.keys[i]].type == "quote"
                    ) {
                        // TODO: Format
                        value += parent.elementList[parent.keys[i]].elem.value;
                    } else if (
                        parent.elementList[parent.keys[i]].type == "title"
                    ) {
                        value +=
                            parent.elementList[
                                parent.keys[i]
                            ].elem.value.toProperCase();
                    } else if (
                        parent.elementList[parent.keys[i]].type == "upper"
                    ) {
                        value +=
                            parent.elementList[
                                parent.keys[i]
